  ...
  return (
    <>
      <div className='flex items-center gap-1 bg-white border-black border-[1px] rounded-2xl px-2 pt-2 pb-2 ml-2 mr-2'>
        ...
        >
          {!isAdmin ? (
            <u className='hover:text-blue-600'>{userInfo?.fullName}</u>
          ) : (
            <u className='hover:text-blue-600'>{userInfo?.fullName}</u>
          )}
        </p>

        <button className='text-sm text-black ml-2 bg-white py-1 px-1 border border-black rounded-lg hover:bg-slate-200' 
          onClick={onLogout} 
        >
          Logout
        </button>
