  ...
  useEffect(() => {
    if (user) {
      ...
      const selectedDept = departments.find(
        (dept) => dept.deptName === user.departments[0]?.name
      );
        ...
        selectedDept.labs.forEach((lab) => {
          if (userLabs.includes(lab.labName)) {
            ...
            lab.sections.forEach((section) => {
              const userLab = user.departments[0].labs.find(
                (userLab) => userLab.name === lab.labName
              );
              if (userLab) {
                const isSelected = userLab.sections.some(
                  (sec) => sec.name === section.sectionName
                );
                if (isSelected) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        role,
        department:
          selectedDepartment && !isAdmin ? selectedDepartment.deptName : null,
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full sm:max-w-md md:max-w-2xl lg:max-w-3xl xl:max-w-4xl max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="mb-4">
          <label className="block mb-2 mt-1 text-xl font-medium">Email</label>
          <input
            type="email"
          ...
          <label className="block mb-2 mt-1 text-xl font-medium">Role</label>
          <select
            className="border-slate-400 border bg-slate-50 p-2 mb-2 w-full rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            value={role}
        ...
        {!isAdmin && (
          <div>
            ...
            {selectedDepartment && (
              <div>
                <h3 className="text-xl font-semibold mb-2">
                  {selectedDepartment.deptName} Labs:
                </h3>
                      ...
                      <div className="ml-6 ">
                        <div className="flex flex-wrap gap-6">
                          {lab.sections.map((section) => (
                            <label
                              key={section._id}
                              className="flex items-center text-lg font-medium mb-1"
                            >
                              <input
                                type="checkbox"
                                className="mr-2 w-5 h-5 align-middle  cursor-pointer"
        ...
        <div className="mt-6">
          ...
          {user && (
            <div className="mt-6 p-4 rounded bg-red-100 border border-red-300">
              ...
              <p className="mb-2">
                To delete this user, please enter their name below for
                confirmation.
              </p>
              <input
                type="text"
